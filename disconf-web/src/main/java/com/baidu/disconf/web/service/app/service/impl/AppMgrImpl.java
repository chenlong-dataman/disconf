...
public class AppMgrImpl implements AppMgr {
    ...
    public List<AppListVo> getAuthAppVoList() {
        ...
        for (App app : apps) {
            ...
            appListVo.setCreateTime(app.getCreateTime());
            appListVo.setUpdateTime(app.getUpdateTime());
            appListVo.setEmails(app.getEmails());
            //----------------------------------
    ...
    @Override
    public String getAppsNameByIds(String ids) {
        String appNames = "";
        ...
        for(String appid : appids) {
            App app = getById(Long.valueOf(appid));
            if(app != null) {
    ...
    @Override
    @Transactional(propagation = Propagation.REQUIRED, readOnly = false)
    public void update(Long appid, String name, String desc, String emails) {
        App app = appDao.get(appid);
        app.setName(name);
        app.setDesc(desc);
        app.setEmails(emails);
        String curTime = DateUtils.format(new Date(), DataFormatConstants.COMMON_TIME_FORMAT);
        app.setUpdateTime(curTime);
